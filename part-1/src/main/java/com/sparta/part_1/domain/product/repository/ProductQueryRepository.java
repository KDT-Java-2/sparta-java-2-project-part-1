...
public class ProductQueryRepository {
  ...
  public Page<ProductResponse> searchProductsPage(ProductSearchRequest request,
    ...
    List<ProductResponse> productResponses = queryFactory.select(
        ...
        .join(category)
        .on(ObjectUtils.isEmpty(product) || ObjectUtils.isEmpty(category) ? null
            : product.category.eq(category), ObjectUtils.isEmpty(request.getCategory()) ? null
  ...
  public List<ProductResponse> searchProductById(Long productId) {
    return queryFactory.select(
            new QProductResponse(product.id, product.name, product.price, product.stock,
                product.category))
        .from(product)
  ...
  private Long calcOffSetNumber(int page, int size) {
    return Long.valueOf((long) page * size - page);
  }
  ...
  public Long findProductCountInCategory(@NotNull Long categoryId) {
